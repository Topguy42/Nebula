...
import { X, ChevronDown, ChevronRight, Monitor, FileText, Bug } from "lucide-react";
...
export default function DevToolsPanel({ isOpen, onClose, iframeUrl }: DevToolsPanelProps) {
      ...
      try {
        ...
        if (iframe) {
          ...
          if (iframeDoc) {
            ...
            setLogs(prev => [...prev, '[INFO] Cross-origin iframe - DOM inspection limited']);
          }
        }
      } catch (e) {
        setLogs(prev => [...prev, `[ERROR] Cannot access iframe content: ${e}`]);
      }
  ...
  const buildDomTree = (element: Element): any => {
    return {
      ...
      children: Array.from(element.children).map(child => buildDomTree(child)),
  ...
  const renderDomNode = (node: any, depth = 0) => {
    ...
    return (
      <div key={`${node.tagName}-${depth}`} style={{ marginLeft: `${depth * 16}px` }}>
        <div 
        ...
        >
          ...
          {node.attributes.slice(0, 2).map((attr: any) => (
            <span key={attr.name} className="text-green-600 dark:text-green-400 text-xs">
              {attr.name}="{attr.value.slice(0, 20)}{attr.value.length > 20 ? '...' : ''}"
            </span>
          ))}
        ...
        {isExpanded && node.children.map((child: any) => renderDomNode(child, depth + 1))}
      </div>
    );
  };
                  ...
                  <div className="ml-4 space-y-1">
                    {selectedElement.attributes.map((attr: any) => (
                      <div key={attr.name} className="text-xs">
                        <span className="text-blue-600 dark:text-blue-400">{attr.name}</span>: 
                        <span className="text-green-600 dark:text-green-400 ml-1">{attr.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <TabsContent value="console" className="flex-1 p-4">
            <ScrollArea className="h-full">
              <div className="space-y-1 font-mono text-sm">
                ...
                <div className="text-blue-600 dark:text-blue-400">Inspecting: {iframeUrl}</div>
                {logs.map((log, index) => (
