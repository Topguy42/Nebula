  ...
  try {
    ...
    try {
      ...
      if (contentType.includes('text/html')) {
        ...
        res.setHeader('Content-Security-Policy', 'frame-ancestors *; default-src * data: blob:; script-src * \'unsafe-inline\' \'unsafe-eval\'; style-src * \'unsafe-inline\';');
...
function processHTML(content: string, targetUrl: URL): string {
  try {
    ...
    if (content.includes('<head>')) {
      ...
      content = content.replace(/<html[^>]*>/i, '$&<head>' + baseTag + '</head>');
    }
